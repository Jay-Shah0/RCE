...
import { TermSocketMessage } from '@/utils/socketEventInterface';
...
const Tabs: React.FC<{ data: TermSocketMessage | null }> = ({ data }) => {
	...
	return (
		<div className="w-full h-full">
			<Tab.Group className="h-full">
				<Tab.List className="flex h-[6vh] py-1 space-x-1 bg-blue-900/20 rounded-lg">
					{tabs.map((tab) => (
						<Tab
							key={tab.id}
							className={({ selected }) =>
				...
				<Tab.Panels className="w-full h-[84vh]">
					{tabs.map((tab) => (
						<Tab.Panel
							key={tab.id}
							className="p-3 bg-white rounded-lg shadow h-full"
						>
							<TerminalComponent
								termId={tab.id.toString()}
								data={termData[tab.id] || ""}
							/>
						</Tab.Panel>
					))}
				</Tab.Panels>
